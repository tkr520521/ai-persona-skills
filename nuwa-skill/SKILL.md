...
本 Skill 的产物是 `*-perspective` 技能目录。安装位置取决于当前运行时：
- Claude Code：`~/.claude/skills/`
- Codex：`$CODEX_HOME/skills/`（默认 `~/.codex/skills/`）
- OpenCode：`~/.config/opencode/skills/`
- Reasonix：`~/.reasonix/skills/`

下文出现的 `.claude/skills/` 是 Claude Code 路径的简写；在 Codex 下替换为 `$CODEX_HOME/skills/`，在 OpenCode 下替换为 `~/.config/opencode/skills/`。
...
**依赖确认**：调用 `gemini-video` / `web-article-reader` / `agent-reach` / `huashu-research` / `pdf` 前，先确认该 skill 在当前运行时的技能目录存在（Claude Code `.claude/skills/`；Codex `$CODEX_HOME/skills/`；OpenCode `~/.config/opencode/skills/`）。缺失时不要假设可用：降级用 WebSearch/标准工具，并在最终交付说明中注明「某 skill 未安装，改用 X」。
...
#### Step 4: 输出
将完成的SKILL.md写入当前运行时的技能目录：Claude Code `.claude/skills/[person-name]-perspective/SKILL.md`；Codex `$CODEX_HOME/skills/[person-name]-perspective/SKILL.md`；OpenCode `~/.config/opencode/skills/[person-name]-perspective/SKILL.md`。
